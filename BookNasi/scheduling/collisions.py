...
def _shorten(facts, *, staff, starts_at, duration_minutes, in_the_way):
    ...
    **Settled 2 August 2026.** A shortened walk-in records `duration_snapshot`
    at the shortened length, `price_snapshot` at the service's full price, and
    `Appointment.was_shortened` to say the two disagree. Time drifts, money does
    not, and the drift is queryable.

    The two alternatives were both worse. Prompting for an adjusted price adds a
    fourth tap to a three-tap flow at the exact moment a staff member is
    resolving a collision with a client standing in front of them, and asks a
    stylist to make the shop owner's pricing decision in public — so the
    realistic outcome is the default accepted every time, which buys the drift
    *and* the tap. Accepting the drift silently keeps the flow fast but makes
    revenue-per-hour quietly flatter whoever shortens most, who is also whoever
    is under most pressure.

    `SHORTEN_FLOOR` still bounds the drift to at most 25% of one appointment's
    time. The flag is what lets slice 9 show it rather than absorb it.
    """
