...
**Per-shop M-Pesa credentials** — settled at slice 13, which built it. This was the last thing on the "still open" list, and it was there because it is a §3 data-model change and a §5 payments change at the same time.

- **The credentials moved onto `Shop`, and the platform till stayed.** `collects_via` is an explicit two-value enum — `own` or `platform` — and not an inference from whether the columns are filled in. That distinction is the whole safety property: a shop that types its shortcode, is interrupted before the passkey, and takes a booking that evening must not collect a real client's deposit into *our* account while every screen reports a healthy booking. So a half-connection is a shop that cannot be booked online, and `shops/readiness.py` says so in words. `shops/migrations/0004` moves every pre-slice-13 shop to `platform`, which is a statement of fact about where their money has been going rather than a policy choice.

- **The env-level block is now the platform account specifically.** It is still required in production, because `platform` is a real destination that a shop can be on and every existing shop is. A deployment where every shop has connected its own would not need it — there is no such deployment, and the switch that would let one say so is config surface bought for a situation nobody is in.

- **Secrets are encrypted in the column, and that is the one dependency added against §11.** `cryptography` is the sixth package in `pyproject.toml`. Neither Django nor the stdlib does authenticated symmetric encryption — `hashlib` and `hmac` verify, `django.core.signing` signs, none of them conceal — so the alternative was hand-rolling AES-GCM, which is the wrong thing to hand-roll because its failure is silent. The threat being addressed is not somebody with a shell on the box, who has the key too; it is every way a *copy* of the database leaves while the key does not: a dump on a laptop, a backup bucket, a restored staging snapshot, a replica handed to an analyst.

  `MPESA_CREDENTIAL_KEYS` is `id:key` pairs, newest first — the first encrypts, all of them decrypt — so a rotation is "add a key at the front" and nothing else, and `Shop.mpesa_key_id` records which one sealed each row so a later re-encryption pass can find them with a `WHERE`. `core/secrets.seal` **refuses when no key is configured** rather than falling back; storing plaintext because configuration was incomplete is the failure this exists to prevent, and it is one that passes every functional test.

- **Owner only — the first endpoint in the product a manager cannot reach.** A manager already sets prices and deposit rules, so they decide how much is taken; where it lands is a different act, and a quiet one, because the number comes back masked and nobody else on the account could see it had changed. `owner_role_required` on `OrgScopedMixin` exists for this one view. `/setup` hides the tab rather than showing a 403, and its M-Pesa request sits outside the batched load — a rejection inside a `Promise.all` would cost a manager their hours, services and staff to a tab they cannot use.

- **What did not change is as load-bearing as what did.** The callback is still one URL with one token: `payments/callbacks.py` finds a payment by `CheckoutRequestID`, which is unique across Safaricom and carries no shortcode, so one endpoint serves any number of tills. A per-shop callback URL would have been a second way to reach the same answer and a new way for one shop's to be wrong. `DarajaClient` already took its config as a constructor argument, so nothing in the code that talks to Safaricom changed at all — `payments/tills.py` just supplies a different dict, which is what slice 6 putting the seam at the interface bought.

  `payments/reconcile.py` did change, and had to: a Daraja query authenticates against the shortcode that took the push, so asking the platform account about a salon's checkout id is a rejection that would have been recorded as "no answer from Safaricom" — a payment stuck `UNKNOWN` forever and a slot released under a client who paid.

- **A shop with no account fails `PUSH_FAILED`, never `UNKNOWN`.** `ShopCannotCollect` subclasses `DarajaMisconfigured`, so it inherits slice 6's rule: nothing was sent, nothing will be until a person changes a setting, and the reconciliation sweep must not chase it. `UNKNOWN` means "the prompt may be on the phone and we cannot tell", and it is not that.

**Data protection** — settled at slice 14, which built §9. Thirteen slices treated the compliance baseline as a constraint to respect rather than a feature to ship, and none of the four obligations existed in code.

- **Erasure is a scrub, and the appointments keep pointing at the scrubbed row.** §9's own wording is precise — "must not orphan appointment records in a way that breaks reporting" — and nulling the link would orphan them. The repeat-client rate groups visits by client, so an erased regular's eight visits would stop being one person's and the figure would move for a reason no owner could find. The row that remains holds no name, no number and no notes; the grouping survives and the person does not.

  A related thing was nearly got wrong and is worth recording: `Credit.client` was going to be relaxed from `PROTECT` to `SET_NULL` so that a client holding an unspent balance could be erased. That was wrong. `PROTECT` fires on a row deletion and erasure never deletes a row, so it was never in the way — relaxing it would have given up a real guard against a hard delete in exchange for nothing. The test suite caught it.

- **What is scrubbed is every copy, and the one that matters is not on the client.** `Payment.phone` is the number Safaricom pushed to, on the table kept longest, and on a model nobody thinks of as holding personal data. An erasure that stopped at the client row would look complete on every screen. Manage tokens are revoked too: a token *is* the session (§12), so a live link in an old SMS is a working credential belonging to somebody who asked to be forgotten — and the page it opens no longer carries their name, which makes it worse rather than better.

- **Retention is 24 months after the last appointment**, enforced by a daily sweep rather than stated in a policy document. A client with no appointments at all is measured from `created_at`, because a row created by a booking that never completed would otherwise live forever — the opposite of a retention policy, applied to the records with the least reason to exist. `CLIENT_RETENTION_MONTHS` can be shortened; there is deliberately no way to switch it off, because "kept indefinitely" is the one answer §9 does not allow. `erasure.retention_statement` is the single wording, the same rule §12 applies to the refund sentence.

- **Unspent credit is voided, and the amount is on the confirm screen before anybody presses anything.** A credit is redeemed by pushing to a phone number; erase the number and it cannot be reached, so holding it would be a liability on the shop's books that can never be discharged. `CreditState.VOIDED_ON_ERASURE` is distinct from `CANCELLED` because `CANCELLED` means the shop decided, and the shop did not decide this.

- **The client can ask, and asking is not erasing.** `POST /api/public/v1/manage/<token>/forget-me/` records a request. The token proves control of the phone — the same verification the deposit relies on — but a one-tap irreversible erase behind an SMS link is too easy to hit by accident, and unlike cancelling there is no version of it the shop can undo. It is also the controller's obligation to discharge and their record to hold: a silent automatic scrub would leave them nothing to show a regulator but an absence. Idempotent, keeping the first timestamp, because the statutory clock starts when the person asked — restarting it on a second tap would let a shop reset its own deadline by prompting.

- **Erasing is owner-only; reading and exporting are managing roles.** Same placement as slice 13's M-Pesa credentials and for a related reason: it cannot be undone and it is quiet, since the row that remains looks like any other erased row. A manager can see the requests and export, because they are the one who will field the phone call, and gets a sentence where the button would be.

- **The export is a file, and its filename is an id.** JSON rather than CSV — the data is nested, appointments with their payments, and a single CSV cannot express that without flattening money onto every row. An erased client still exports, truthfully, saying what is left and why; a 404 there would read as evasion.

### Still open — do not silently decide these

Nothing at present. The four questions this section has carried — the refund terms, the `slotLost` remedy, per-shop M-Pesa credentials, and §9's export and delete paths — are all settled above, with the reasoning kept rather than deleted so they are not reopened by accident.
